...
def save_video_file(
    ...
    *,
    ...
    filename = secure_filename(file.filename or "")
    if not filename or "." not in filename:
        raise BadRequestException(message="invalid video file")

    extension = filename.rsplit(".", 1)[1].lower()
    if extension not in allowed_extensions:
        raise BadRequestException(message="unsupported video type")

    file.stream.seek(0, 2)
    size_bytes = file.stream.tell()
    file.stream.seek(0)
    if size_bytes <= 0:
        raise BadRequestException(message="empty video file")
    if size_bytes > max_bytes:
        raise BadRequestException(message="video file too large")
